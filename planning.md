...
- **Step 4 — Public booking page (Book.jsx):** Calendar and time slots use theme spacing and typography; slot chips use `theme.secondaryBg` for a tappable look; form step and summary card use theme spacing/fontSize; success block and add-to-calendar use theme tokens.
- **Step 5 — Instructor dashboard:** Sidebar nav active state has a left border (`3px solid` primary) for clarity; SetupHome event cards have subtle hover shadow via `[data-card-hover]` in `index.css`; SignInPage and SignUpPage wrappers use `theme.background` and `theme.text` so auth pages match the app.

### Implementation steps (executed for Steps 1–5)
...
2. **Replace hardcoded colors** — InstructorLayout: `navItemActive` uses `theme.navActiveBg`. SetupHome: `previewBtn`, `copyBtn` use `theme.secondaryBg`. Book and SetupEventForm: use theme tokens.
3. **Align global CSS** — In `index.css`, set body `background` and `color` to match theme. Add Inter font link in `index.html` and set `font-family: 'Inter', system-ui, ...` on body.
...
5. **Focus and transitions** — Add global `:focus-visible` rule in `index.css` using primary color. Add `theme.transition` to interactive elements.
6. **Step 4 (Book.jsx):** Page/card/summary and error styles use theme.spacing and theme.fontSize; slot chips use `theme.secondaryBg`; timeZoneRow and success block use theme tokens; addToCalendar uses theme.fontSize and transition.
7. **Step 5 (dashboard):** InstructorLayout `navItemActive` gets left border and marginLeft for clear active state. SetupHome event cards get `data-card-hover` and `index.css` rule for subtle hover shadow; eventCard style gets transition. SignInPage and SignUpPage wrappers use theme.background, theme.text, theme.spacing[24].
...
| Global   | `client/index.html` — Inter font link. `client/src/index.css` — body background/color/font; :focus-visible; hover; [data-card-hover]:hover. |
| Layout   | `client/src/components/InstructorLayout.jsx` — theme tokens, spacing, fontSize, transition, navItemActive left border. |
| Pages    | `client/src/pages/SetupHome.jsx` — secondaryBg, spacing, fontSize, transition, eventCard transition, data-card-hover. |
| Pages    | `client/src/pages/Book.jsx` — theme tokens, fontSize, spacing, transition, secondaryBg for slotChip, summary/success/addToCalendar. |
| Pages    | `client/src/pages/SetupEventForm.jsx` — theme fontSize, spacing, transition on inputs/buttons. |
| Pages    | `client/src/pages/SignInPage.jsx`, `SignUpPage.jsx` — wrapper with theme.background, theme.text, theme.spacing. |
